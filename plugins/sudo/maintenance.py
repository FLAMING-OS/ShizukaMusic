...
from modules.utils.database import (get_lang, is_maintenance,
                                       maintenance_off,
                                       maintenance_on)
    ...
    if state == "enable":
        if await is_maintenance() is False:
            await message.reply_text(
                "Bgt Music Bot Maintenance mode is already enabled"
            )
        else:
        ...
        if await is_maintenance() is False:
            ...
            await message.reply_text(
                "Bgt Music Bot Maintenance mode is already disabled"
            )
    else:
